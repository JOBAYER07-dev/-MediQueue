    ...
    >
      ...
      <div className="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-12 items-center py-16">
        ...
        <div key={current} style={{ animation: 'fadeUp .5s ease both' }}>
          ...
          <h1 className="font-serif text-[clamp(2.4rem,5vw,3.8rem)] font-black leading-[1.06] tracking-[-1.5px] text-[#e8ecf4] mb-6">
            {slide.title}
            <br />
        ...
        {/* Right — Floating Card */}
        <div className="relative bg-[#131829] border border-white/[0.07] rounded-2xl p-6 shadow-2xl hidden lg:block">
          ...
          <div className="grid grid-cols-2 gap-3 mb-5">
            {[
              ...
              >
                ...
                <div
                  className={`text-[0.84rem] font-semibold ${gold ? 'text-[#d4a84b]' : 'text-[#e8ecf4]'}`}
                >
                  {value}
                </div>
              </div>
          ...
          <div className="flex justify-center gap-2 mt-5">
            {slides.map((_, i) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all duration-300 ${i === current ? 'w-6 bg-[#d4a84b]' : 'w-1.5 bg-white/20'}`}
              />
            ))}
          </div>
        </div>
